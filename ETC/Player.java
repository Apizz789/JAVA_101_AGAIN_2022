...
abstract class Player {
    ...
    public Player() {
        ...
        // each addStr() increase str

        dexPoint = dex * 2;
        dexPoint = weapon != null ? dexPoint + weapon.getDex() : dexPoint;
        // each addDex() increase chance of hitting opponent 2%
        if (dexPoint > 100)
            dexPoint = 100;

        health = 100 + vit * 5;
        // each addVit() increase health 5 points (health starts with 100)
    }

    protected abstract void setName(); /* call addStr(), addDex(), or addVit() here only */

    protected abstract void setColor(); /* new your Weapon of choice */
